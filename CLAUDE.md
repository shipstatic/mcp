...
`package.json` is the single source of truth for the version — CI patches both `.version` and `.packages[0].version` in `server.json` with `jq` before registry publish. **The version in the tracked `server.json` is therefore a placeholder and is stale on purpose** (it reads `0.2.0`); never run `mcp-publisher publish` by hand from a checkout, which would register that literal. DNS authentication uses an Ed25519 key on `shipstatic.com`.
